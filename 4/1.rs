...
fn board_iter(board: &Vec::<i32>, moves: &HashSet<i32>,
    idxr: &dyn Fn(usize, usize)->usize) -> bool {
  for i in 0..5 {
    let mut count: i32 = 0;
    for j in 0..5 {
...
fn main() {
  ...
  let moves: Vec::<i32> = moves_line.trim().split(",").map(|s| s.parse().unwrap()).collect();
  ...
  'outer: for i in 1..moves.len() {
    for board in &boards {
      let board_sum = bingo(&board, &moves[..=i]);
      if board_sum != -1 {
